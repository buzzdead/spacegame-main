 ...
 const [smoke, setSmoke] = useState(false)
  ...
  return (
    ...
    >
      ...
      <PhasedShips resetPortal={resetPortal} shouldMove={isOpen} pos={position} />
    </RoundedBox>
  );
};
...
const SpawnPortal = React.memo(
  Portal,
  (prevProps, nextProps) => prevProps.pos === nextProps.pos
);
  ...
  return (
    <group>
      {phasedShips.map((ps, id) => {
        return ps.stop || !shouldMove ? null : <primitive key={id} object={ps.ship} />;
      })}
    </group>
  );
};
