...
export const Explosion = ({ position }: Props) => {
  ...
  useFrame(() => {
    const updateParticles = () => {
      if (particleSystemRef.current && particleSystemRef.current.geometry.attributes.position && particleSystemRef.current.geometry.attributes.velocity) {
        ...
        for (let i = 0; i < positions.length; i += 3) {
          ...
          velocities[i + 1] += direction.y * 0.0001 + (Math.random() - 0.5) * 0.001;
          velocities[i + 2] += direction.z * 0.0001 + (Math.random() - 0.5) * 0.001;
          ...
          if(material instanceof THREE.Material && material.opacity > 0.0025) {material.opacity -= 0.0000025;}
