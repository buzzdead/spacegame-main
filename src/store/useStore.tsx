...
export interface SpaceGameState {
  ...
  selectedEnemies: any;
  setSelectedEnemies: (a: any) => void;
  dealDamageToEnemy: (pos: Vector3, n: number) => boolean;
  resources: number;
...
const useStore = create<SpaceGameState>((set) => ({
  ...
  selectedEnemies: [],
  setSelectedEnemies: (a: any) => set((state) => ({selectedEnemies: a })),
  dealDamageToEnemy: (pos: Vector3, n: number) => {
    let destroyed = false;
    set((state) => {
      const newHull = state.selectedEnemies?.hull - n;
      destroyed = newHull <= 10;
      return ({ selectedEnemies: { ...state.selectedEnemies, hull: newHull } });
    });
    return destroyed;
  },
  resources: 1500,
